...
if(summarize_1000_5000_nregpath_warm)
    ...
    for k=1:1:kmax
        ...
        norm_dual_glmnet_incl(k) = norm(sol_glmnet_p(:,k)-sol_incl_p(:,k),2)/norm(sol_glmnet_p(:,k),2);
        norm_primal_glmnet_incl(k) = norm(sol_glmnet_x(:,k)-sol_incl_x(:,k),2)/norm(sol_glmnet_x(:,k),2);
    end
    ...
    disp('l2 norm of the differences between primal solutions (glmnet/incl first).')
    disp([norm_dual_glmnet_incl, norm_dual_fista_incl,norm_dual_fista_glmnet])

    disp('l2 norm of the differences between dual solutions (glmnet/incl first).')
    disp([norm_dual_glmnet_incl, norm_dual_fista_incl,norm_dual_fista_glmnet])
end