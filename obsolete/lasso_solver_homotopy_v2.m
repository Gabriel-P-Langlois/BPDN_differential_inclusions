...
while(shall_continue && k <= buffer_path)
    ...
    if(~isempty(ind_flag))
        tmp = find(equicorrelation_set);
        equicorrelation_set(tmp(ind_flag)) = false;
    end
    ...
    u = lsqnonneg(K*D,b);
    if(k == 29)
        disp(['Displaying index: ',num2str(k)])
    ...
    if (any(active_set_plus))
        ...
        timestep = min(vec);
        if(timestep < 0)
            disp(['Problem at iteration #: ',num2str(k)])
        end
