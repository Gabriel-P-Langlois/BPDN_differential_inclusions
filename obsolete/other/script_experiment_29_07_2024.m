...
% Dimensions -- Number of samples and features
m = 100; n = 500;

% Exact homotopy algorithm
tol_exact = 1e-08;              % Tolerance
disp_output_exact = false;       % Display output information
run_opt_cond_checks = false;    % Perform various checks to verify that
                                % the optimality conditions are OK.
                                % Note: Will slow down the algorithm
...
use_glmnet = true;  
if(use_glmnet)
    tol_glmnet = tol_exact;     % Relative tolerance of GLMNET
end

% Run diagnostics at the end (see last section at the end).
...
if(use_bp)
    ...
    % Call the BP solver
    tic
    [sol_x_bp, sol_p_bp] = BP_exact_algorithm(A,b,tol_exact,disp_output_bp);
    time_bp = toc;
