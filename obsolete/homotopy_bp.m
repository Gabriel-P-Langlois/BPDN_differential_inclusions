...
function [sol_x,sol_p,sol_t] = ...
    homotopy_bp(A,b,m,n,tol,display_iterations,run_diagnostics)
...
while(shall_continue && k <= buffer_path)
    ...
    %%%%% 2. Solve the LSQ problem and compute the descent direction
    u = lsqnonneg(K*D,b);
    v = zeros(n,1); v(equi_set) = D*u;
    d = K*(D*u) - b;


    %%%%% 3. Compute the kick time
    ...
    if (any(active_set_plus))
        ...
        %%%%% 4. Update all solutions and variables
