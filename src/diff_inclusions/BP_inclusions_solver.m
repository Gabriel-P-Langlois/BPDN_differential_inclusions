function [sol_x, sol_p] = BP_inclusions_solver(A,b,p0,tol)
...
while(true)
    ...
    K = A(:,new_eq_set).*(vec_of_signs(new_eq_set).');
    ...
    if(length(ind) >= 1)
        for k=1:1:length(ind)
            ...
            loc = find(find(new_eq_set) == ind(k));
            ...
            % The following is equivalent to [Q,R] = qrinsert(Q,R,loc,col);
