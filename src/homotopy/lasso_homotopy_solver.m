%% Description -- lasso_homotopy_solver_opt
%
% Exact homotopy algorithm -- Written by Gabriel Provencher Langlois
%
% This solves a least-squares problem at every iterations

function [sol_exact_x,sol_exact_p,exact_path] = ...
    lasso_homotopy_solver(A,b,tol,display_iterations)


%% Initialization
m = size(A,1); n = size(A,2);
sol_exact_p = zeros(m,m+1); sol_exact_x = zeros(n,m+1);
exact_path = zeros(m+1,1);
tol_minus = 1-tol;

bminus = -b;
bminust = bminus.';
tmp1 = (bminust*A).';
t0 = norm(tmp1,inf);
tmp1 = tmp1/t0;

moving_term_1 = [tmp1;-tmp1];
active_set = (moving_term_1 >= tol_minus); % Checks for 

% Store first time and first dual solution
exact_path(1) = t0;
sol_exact_p(:,1) = bminus/t0;


%% The algorithm
shall_continue = true;
i = 0; max_iters = m;

while(shall_continue && i < max_iters)
    i=i+1;

    %%% 1. Cone projection
    % We compute min_{x} normsq{A_{active}*x + b}
    K = [A(:,active_set(1:n)),-A(:,active_set(n+1:end))];
    u_active = K\b;

    % Express solution direction back to Rn coordinates
    tmp = zeros(2*n,1); tmp(active_set) = u_active;
    x_active = tmp(1:end/2) - tmp(end/2+1:end);

    %%% 2. Compute the descent direction = -(Aeff*xeff+b)
    direction = (K*u_active + bminus);

    %%% 3. Compute kick time (A.'*direction)
    tmp2 = (direction.'*A).';
    moving_term_2 = [tmp2;-tmp2]; 

    active_set_plus = (moving_term_2 > tol);
    if (any(active_set_plus))
        vec = -(moving_term_1-1)./moving_term_2;
        timestep = min(vec(active_set_plus));
    else % Stop; termination condition satisfied.
        sol_exact_x(:,i+1) = x_active; 
        break;
    end


    %%% 5. Update dual variable and regularization path
    % Update dual solution + time parameter
    sol_exact_p(:,i+1) = sol_exact_p(:,i) + timestep*direction;
    exact_path(i + 1) = exact_path(i)/(1+exact_path(i)*timestep);

    % Update primal solution  using d_active
    alpha = exact_path(i+1)/exact_path(i);
    sol_exact_x(:,i+1) = alpha*sol_exact_x(:,i) + (1-alpha)*x_active; 

    %%% 6. Compute the active set for the next iteration
    moving_term_1 = moving_term_1 + timestep*moving_term_2;
    active_set = (moving_term_1 >= tol_minus);

    %%% 7. If enabled, display current iteration
    if(display_iterations)
        disp(['Iteration ',num2str(i),' is complete.'])
    end
end
end


%%% BACKUP
%% Description -- lasso_homotopy_solver_opt
% %
% % Exact homotopy algorithm -- Written by Gabriel Provencher Langlois
% %
% % This code is based on present research done by the authors and Ciril et
% % al. (2021).
% 
% function [sol_exact_x,sol_exact_p,exact_path] = lasso_homotopy_solver(A,b,tol)
% 
% 
% %% Initialization
% m = size(A,1); n = size(A,2);
% sol_exact_p = zeros(m,m+1); sol_exact_x = zeros(n,m+1);
% exact_path = zeros(m+1,1);
% tol_minus = 1-tol;
% 
% bminus = -b;
% bminust = bminus.';
% tmp1 = (bminust*A).';
% t0 = norm(tmp1,inf);
% tmp1 = tmp1/t0;
% 
% moving_term_1 = [tmp1;-tmp1];
% active_set = (moving_term_1 >= tol_minus);
% 
% % Store first time and first dual solution
% exact_path(1) = t0;
% sol_exact_p(:,1) = bminus/t0;
% 
% 
% %% The algorithm
% shall_continue = true;
% i = 0; max_iters = m;
% 
% while(shall_continue && i < max_iters)
%     i=i+1;
% 
%     %%% 1. Cone projection
%     % We compute min_{x} normsq{A_{active}*x + b}
%     K = [A(:,active_set(1:n)),-A(:,active_set(n+1:end))];
%     x_active = K\bminus;
% 
%     % Express solution direction back to Rn coordinates
%     tmp = zeros(2*n,1); tmp(active_set) = x_active;
%     d_active = -tmp(1:end/2) + tmp(end/2+1:end);
% 
%     %%% 2. Compute the descent direction = -(Aeff*xeff+b)
%     direction = (K*(-x_active) + bminus);
% 
%     %%% 3. Compute kick time (A.'*direction)
%     tmp2 = (direction.'*A).';                   % SLOW LINE
%     moving_term_2 = [tmp2;-tmp2]; 
% 
%     disp(vecnorm(moving_term_2(active_set),2)) % seems to be always zero
% 
%     active_set_plus = (moving_term_2 > tol);
%     if (any(active_set_plus))
%         vec = -(moving_term_1-1)./moving_term_2;
%         timestep = min(vec(active_set_plus));
%     else % Stop; termination condition satisfied.
%         sol_exact_x(:,i+1) = d_active; 
%         break;
%     end
% 
% 
%     %%% 5. Update dual variable and regularization path
%     % Update dual solution + time parameter
%     sol_exact_p(:,i+1) = sol_exact_p(:,i) + timestep*direction;
%     exact_path(i + 1) = exact_path(i)/(1+exact_path(i)*timestep);
% 
%     % Update primal solution  using d_active
%     alpha = exact_path(i+1)/exact_path(i);
%     sol_exact_x(:,i+1) = alpha*sol_exact_x(:,i) + (1-alpha)*d_active; 
% 
%     %%% 6. Compute the active set for the next iteration
%     moving_term_1 = moving_term_1 + timestep*moving_term_2;
%     active_set = (moving_term_1 >= tol_minus);
% end
% end