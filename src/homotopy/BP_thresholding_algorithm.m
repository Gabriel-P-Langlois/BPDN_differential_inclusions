function [sol_x, sol_p, sol_t, xtot, b_vec] = BP_thresholding_algorithm(A,b,tol,disp_output_bp)
%% Description -- BP_thresholding_algorithm
% Version of the algorithm where w == max(0,tmp0);
...
for i=1:1:m
    ...
    % Choose w to be thresholded by -max(0,-tmp0)
    w_vec(equi_set,i) = -max(0,-tmp0);
    q_vec(:,i) = A*(D.*w_vec(:,i));
